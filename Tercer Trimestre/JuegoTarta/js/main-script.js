...
let numComensalesActual = parseFloat(
  document.querySelector("#numcomensales").textContent,
);

const baseZanahoria =
  parseFloat(document.getElementById("zanahoria").textContent) /
  numComensalesActual;
const baseHarina =
  parseFloat(document.querySelector("#harina").textContent) /
  numComensalesActual;
const baseAzucar =
  parseFloat(document.querySelector("#azucar").textContent) /
  numComensalesActual;
const baseHuevos =
  parseFloat(document.querySelector("#huevos").textContent) /
  numComensalesActual;
const baseAceite =
  parseFloat(document.querySelector("#aceite").textContent) /
  numComensalesActual;
const basePolvo =
  parseFloat(document.querySelector("#polvo").textContent) /
  numComensalesActual;
const baseCanela =
  parseFloat(document.querySelector("#canela").textContent) /
  numComensalesActual;
const baseNuez =
  parseFloat(document.querySelector("#nuez").textContent) / numComensalesActual;
const baseSal =
  parseFloat(document.querySelector("#sal").textContent) / numComensalesActual;
...
function actualizarReceta(nuevosComensales) {
  ...
  document.getElementById("zanahoria").innerText =
    baseZanahoria * nuevosComensales;
  ...
  switch (cucharaditas) {
    ...
    default:
      ...
      document.getElementById("canela").innerText =
        baseCanela * nuevosComensales;
...
imgSuma.addEventListener("click", () =>
  actualizarReceta(numComensalesActual + 1),
);
imgResta.addEventListener("click", () =>
  actualizarReceta(numComensalesActual - 1),
);
